...
#Institutions: British Geological Survey, Kelpie Geosciences
#CoMMa version 1.3 - 2026
...
from arcpy.ia import ElevationVoidFill
...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [Basic_descriptor, Texture_descriptor, Volume_descriptor, Point_descriptor]
...
class helpers(object):
    ...
    def AVI(self,out_slope,out_aspect):
        ...
        try:
            ...
            out_int_aspect = Int(out_aspect)
                
            out_Var_aspect = FocalStatistics(out_int_aspect, neighborhood, "VARIETY")
    ...
    def euos_decimalcomma_to_float(self, value):
        try:
            return float(value)
        
        except ValueError:
            try:
                # Replace commas with dots for EU OS and attempt conversion
                return float(value.replace(',', '.'))
            
            except ValueError:
                try:
                    # Extract the integer part and convert to float
                    int_part = ''.join(c for c in value if c.isdigit())
                    return float(int_part)
                
                except ValueError:
                    # Final error message if all attempts fail
                    raise ValueError("Decimal point issue: decimal points must be dots.")
...
class Basic_descriptor(object):
    ...
    def execute(self, parameters, messages):
        ...
        Cellsize_X2 = (helper.euos_decimalcomma_to_float(Cellsize_Xresult.getOutput(0)))**2
        ...
        for ra in List_dev:
            ...
            arcpy.AlterField_management(base_lyr, "MIN", f"{ra_name}MIN", f"{ra_name}MIN")
            
            roundExpression = f"round(!{ra_name}MIN!, 2)"
            arcpy.CalculateField_management(base_lyr, f"{ra_name}MIN", roundExpression, "PYTHON3")
            
            arcpy.AlterField_management(base_lyr, "MAX", "{}MAX".format(ra_name), "{}MAX".format(ra_name))
            
            roundExpression = f"round(!{ra_name}MAX!, 2)"
            arcpy.CalculateField_management(base_lyr, f"{ra_name}MAX", roundExpression, "PYTHON3")
            
            arcpy.AlterField_management(base_lyr, "MEAN", "{}MEAN".format(ra_name), "{}MEAN".format(ra_name))
            
            roundExpression = f"round(!{ra_name}MEAN!, 2)"
            arcpy.CalculateField_management(base_lyr, f"{ra_name}MEAN", roundExpression, "PYTHON3")
        ...
        if parameters[5].value is not None:
            ...
            remapFOSL = "1 7 NODATA;8 1; 9 10 NODATA"
            remapVALL = "1 8 NODATA;9 1; 10 NODATA"
            remapPIT = "1 9 NODATA;10 1"
            
            clip_geom = ExtractByMask(inputGeo, tempWS + "deli.shp")
            List_gmre = [remapFLAT, remapPEAK, remapRID, remapSHOU, remapSPUR, remapSLO, 
                         remapFOSL, remapHOLL, remapVALL, remapPIT]
            List_gmnam = ["gm_flat", "gm_peak", "gm_ridg", "gm_shou", "gm_spur", 
                          "gm_slop", "gm_fosl", "gm_holl", "gm_vall", 
                          "gm_pit"]
            ...
            for gm_re in List_gmre:
                ...
                geom_poly_clip = tempWS + "geom_calc.gdb/{}_c".format(gmra_name)
                arcpy.analysis.Clip(geom_poly, base_lyr, geom_poly_clip)
                
                field_gm_poly = []
                for field in arcpy.ListFields(geom_poly_clip):
                    if not field.required:
                        field_gm_poly.append(field.name)
                arcpy.DeleteField_management(geom_poly_clip, field_gm_poly)
                
                arcpy.AddField_management(geom_poly_clip, "Area_gm", "DOUBLE", "8", "2")
                areaExpression = "float(!SHAPE.AREA!)"
                arcpy.CalculateField_management(geom_poly_clip, "Area_gm", areaExpression, "PYTHON3")
                
                arcpy.SpatialJoin_analysis(base_lyr, geom_poly_clip, tempWS + "geom_calc.gdb/{}_pj".format(gmra_name),
        ...
        keepFields.extend(["Geo_rat","Conf_R","Slope_MIN","Slope_MAX",
                           "Slope_MEAN","LDfG_MAX","LDfG_MEAN","Depth_MIN","Depth_MAX",
                           "Depth_MEAN","prc_peak","prc_ridg","prc_shou","prc_spur","prc_slop", "prc_fosl",
        ...
        arcpy.AddField_management(tempWS + "joi.shp", "MBG_W_L", "DOUBLE", "8", "3")
        arcpy.CalculateField_management(tempWS + "joi.shp", "MBG_W_L", '!MBG_Width!/!MBG_Length!', "PYTHON3")
        ...
        arcpy.CalculateField_management(tempWS + "b.shp", "PP_Score", '4*3.1492*!Area!/((!Perimeter!*!Perimeter!)+0.01)', "PYTHON3")
        ...
        keepFields.extend(["Perimeter","Area","Area_CH","CH_Score","Dissect","Depth_RAN", "MBG_Width",
                           "MBG_Length","MBG_W_L","MBG_Orient","PP_Score","LDfG_VAR","QuLDfG_Ran"])
        ...
        try:
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            aprx.activeMap.addDataFromPath(workspace + "/" + outFeat)
        except:
            pass
...
class Texture_descriptor(object):
    ...
    def execute(self, parameters, messages):
        ...
        with arcpy.EnvManager(scratchWorkspace=tempWS):
            ...
            out_raster.save(tempWS + "Asp.tif")
        
        #Aspect_ = Aspect(inputDEM)
        Aspect_ = Raster(tempWS + "Asp.tif")
...
class Volume_descriptor(object):
    ...
    def getParameterInfo(self):
        ...
        param3 = arcpy.Parameter(
            displayName="Choose the interpolation method for the base surface",
            name="vol_method",
            datatype="GPString",
            parameterType="Required",
            direction="Input")
        ...
        param3.filter.list = ["Iterative spline (slow)", "Global spline", "Plane fitting/IDW (fast)"]
        
        param4 = arcpy.Parameter(
            displayName="Spline Tension",
            name="spline_tens",
            datatype="GPLong",
            parameterType="Optional",
            direction="Input")
    ...
    def updateParameters(self, parameters):
        ...
        if parameters[3].value == "Iterative spline (slow)" or parameters[3].value == "Global spline":
    ...
    def execute(self, parameters, messages):
        ...
        spline_tens = parameters[4].valueAsText
        workspace = parameters[5].valueAsText
        outFeat = parameters[6].valueAsText
        delTemp = parameters[7].valueAsText
        ...
        Cellsize_Xresult = arcpy.GetRasterProperties_management(inputDEM, "CELLSIZEX")
        Cellsize_X = (helper.euos_decimalcomma_to_float(Cellsize_Xresult.getOutput(0)))
        ...
        if method == "Plane fitting/IDW (fast)":
            
            voided = ExtractByMask(ras1, poly1, "OUTSIDE")
            
            arcpy.AddMessage("Filling base surface... ")
            voidfilled = ElevationVoidFill (voided, 0)
            voidfilled.save("evf_fitted")
            
            if fillDirec == "Positive":
                subtracted = Minus(ras1, voidfilled)
                nulledNegatives = SetNull(subtracted, subtracted, "VALUE < 0")
                volumetrics = nulledNegatives * (Cellsize_X**2)
                #subtracted.save("subtracted")
                #nulledNegatives.save("nulledNegatives")
                #volumetrics.save("volumetrics")

            else:
                subtracted = Minus(voidfilled, ras1)
                nulledNegatives = SetNull(subtracted, subtracted, "VALUE < 0")
                volumetrics = nulledNegatives * (Cellsize_X**2)

            arcpy.AddMessage("Extracting statistics... ")
            ZonalStatisticsAsTable(in_zone_data = poly1, 
                                   zone_field = "OBJECTID", 
                                   in_value_raster = volumetrics, 
                                   out_table = "out_tab", 
                                   ignore_nodata = "DATA", 
                                   statistics_type = "SUM")
                       
            volume_join = arcpy.AddJoin_management(poly1, "OBJECTID", 
                                     "out_tab", "OBJECTID", "KEEP_ALL")
            
            arcpy.CopyFeatures_management(volume_join, "poly2")
            
            
            ZonalStatisticsAsTable(in_zone_data = "poly2", 
                                   zone_field = "OBJECTID", 
                                   in_value_raster = subtracted, 
                                   out_table = "out_tab_vr", 
                                   ignore_nodata = "DATA", 
                                   statistics_type = "MAXIMUM")

            omptim_vr_join = arcpy.AddJoin_management("poly2", "OBJECTID", 
                                     "out_tab_vr", "OBJECTID", "KEEP_ALL")
            
            arcpy.CopyFeatures_management(omptim_vr_join, "poly3")
            
            #Save file and final cleaning
            arcpy.AddMessage('Final cleaning ...') 
            arcpy.management.AlterField("poly3", "SUM", "Volume", "Volume")
            arcpy.management.AlterField("poly3", "MAX", "Optim_R", "Optim_R")

            keepFields.extend(["Volume", "Optim_R"])
            helper.field_removal("poly3", keepFields)
            
            arcpy.AddMessage('Saving official shapefile ...')
            arcpy.conversion.ExportFeatures("poly3", workspace + "/" + outFeat, "", "NOT_USE_ALIAS")
        ...
        elif method == "Global spline":
            
            clip = arcpy.Clip_management(ras1, "", 'clip', poly1, '-9999', 'ClippingGeometry')
            
            arcpy.AddMessage("Creating base surface... ")
            voided = ExtractByMask(ras1, poly1, "OUTSIDE")
            buf_dist = str(Cellsize_X*10)
            arcpy.AddMessage(f"Buffering, distance = {buf_dist}")
            arcpy.analysis.Buffer(poly1, "buffer", f"{buf_dist} Meters", "OUTSIDE_ONLY", "ROUND", "ALL")
            
            base_neighbourhood = arcpy.Clip_management(voided, "", 'base_nb', 'buffer', '-9999', 'ClippingGeometry')
                        
            arcpy.AddMessage("Expansion complete... now pointifying")
            #convert the periphery to points
            arcpy.RasterToPoint_conversion(base_neighbourhood, 'base_nb_pt', "VALUE")
            
            arcpy.AddMessage("Points complete... now splining")
            #interpolate using a spline
            outSpline = Spline('base_nb_pt', "grid_code", Cellsize_X, "TENSION", float(spline_tens))

            arcpy.AddMessage("Spline complete...now clipping")
            clip3 = arcpy.Clip_management(outSpline, "", 'splined', poly1, '-9999', 'ClippingGeometry')
            
            arcpy.AddMessage("Clip complete...now calculating volumes")
            ...
            with arcpy.da.UpdateCursor(poly1, ['SHAPE@', 'OBJECTID', 'Optim_R', 'Volume']) as cursor:
                ...
                for row in cursor:
                    ...
                    obj_id = str(row[1])
                    arcpy.AddMessage(f'processing object: {obj_id}')
                    
                    #clip the mound
                    clip = arcpy.Clip_management(ras1, "", 'clip', row[0], '-9999', 'ClippingGeometry')
                    arcpy.AddMessage("Clipping complete...now expanding")
                    ...
                    #out_focal_statistics.save('out_f_st')
                    arcpy.AddMessage("Expanding complete...now pointifying")          
                    ...
                    arcpy.Clip_management(out_focal_statistics, "", 'donutr', 'donut', '-9999', 'ClippingGeometry')
    
                    arcpy.AddMessage("Donut complete... now pointifying")
                    #convert the periphery to points
                    arcpy.RasterToPoint_conversion('donutr', 'donut_p', "VALUE")
                    
                    arcpy.env.extent = 'donutr'
                    arcpy.AddMessage("Donut points complete... now splining")
                    #interpolate using a spline
                    outSpline = Spline('donut_p', "grid_code", Cellsize_X, "TENSION", float(spline_tens))
                    #outSpline.save(f"i_sp_{obj_id}")
    
                    arcpy.AddMessage("Spline complete...now clipping")
                    clip3 = arcpy.Clip_management(outSpline, "", 'splined', row[0], '-9999', 'ClippingGeometry')
                    
                    arcpy.AddMessage("Clip complete...now calculating volume")
                    subtracted = Minus(clip, clip3)
...
class Point_descriptor(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Point extrema extractor"
        self.description = "Find highest or lowest points tool"
        self.canRunInBackground = False
    ...
    def getParameterInfo(self):
        ...
        param2 = arcpy.Parameter(
            displayName="Select the extrema you wish to map",
            name="extr_point",
            datatype="GPString",
            parameterType="Optional",
            direction="Input",
        ...
        param2.filter.type = "ValueList"
        param2.filter.list = ["Highest", "Lowest"]
    ...
    def execute(self, parameters, messages):
        ...
        inputDEM = parameters[0].valueAsText
        in_features = parameters[1].valueAsText
        extrema = parameters[2].valueAsText
        workspace = parameters[3].valueAsText
        outFeat = parameters[4].valueAsText
        delTemp = parameters[5].valueAsText
        ...
        arcpy.CreateFileGDB_management(tempWS[:-1], "extr_calc.gdb")
        
        arcpy.FeatureClassToGeodatabase_conversion(in_features, tempWS + "extr_calc.gdb")
        arcpy.RasterToGeodatabase_conversion(inputDEM, tempWS + "extr_calc.gdb")
        
        arcpy.env.qualifiedFieldNames = False
        #arcpy.env.workspace = workspace
        arcpy.env.workspace = tempWS + "extr_calc.gdb"
        
        for fc in arcpy.ListFeatureClasses():
            poly1 = tempWS + "extr_calc.gdb/{}".format(fc)
            arcpy.AddMessage(fc)
        for raster in arcpy.ListRasters():
            arcpy.AddMessage(raster)
            ras1 = tempWS + "extr_calc.gdb/{}".format(raster)
            

        extrema_list = [e.strip() for e in str(extrema).split(";") if e.strip()]
        do_high = "Highest" in extrema_list
        do_low = "Lowest" in extrema_list
    
        zone_field = arcpy.Describe(poly1).OIDFieldName
        
        arcpy.AddMessage("Calculating extrema...")
        
        
        def make_output_name(kind):

            out_base = outFeat[:-4] if outFeat.lower().endswith(".shp") else outFeat

            if do_high and do_low:
                return f"{out_base}_{kind}.shp"
            else:
                return f"{out_base}.shp"
            
        
        def zonal_extrema_to_points(stat_kind):

            if stat_kind == "Highest":
                stat_type = "MAXIMUM"
                suffix = "high"
            else:
                stat_type = "MINIMUM"
            ...
            # Temp dataset names in the temp GDB
            pts_raw   = os.path.join(arcpy.env.workspace, f"pts_depth_{suffix}")
            pts_join  = os.path.join(arcpy.env.workspace, f"pts_depth_{suffix}_sj")

            zs = ZonalStatistics(
                in_zone_data=poly1,
                zone_field=zone_field,
                in_value_raster=ras1,
                statistics_type=stat_type,
                ignore_nodata="DATA")

            cells = Con(ras1 == zs, ras1)

            arcpy.conversion.RasterToPoint(cells, pts_raw, "VALUE")
            
            arcpy.analysis.SpatialJoin(
                target_features=pts_raw,
                join_features=poly1,
                out_feature_class=pts_join,
                join_operation="JOIN_ONE_TO_MANY",
                join_type="KEEP_COMMON",
                match_option="INTERSECT")
            
            arcpy.management.DeleteIdentical(pts_join, "JOIN_FID")
            arcpy.management.AlterField(pts_join, "grid_code", "pt_{suffix}_h", "pt_{suffix}_h")
            arcpy.management.DeleteField(pts_join, ["Join_Count", "pointid", "TARGET_FID", "JOIN_FID"])
            
            out_name = make_output_name(stat_kind)
            out_path = os.path.join(workspace, out_name)

            arcpy.conversion.FeatureClassToFeatureClass(
                in_features=pts_join,
                out_path=workspace,
                out_name=out_name)
        ...
        if do_high:
            arcpy.AddMessage("Finding highest points...")
            outputs.append(zonal_extrema_to_points("Highest"))

        if do_low:
            arcpy.AddMessage("Finding lowest points...")
            outputs.append(zonal_extrema_to_points("Lowest"))


        for p in outputs:
            arcpy.AddMessage(f"Created: {p}")
