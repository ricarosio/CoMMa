...
class helpers(object):
    ...
    def euos_decimalcomma_check(self, value):
        try:
            float(value)
            return value
...
class Delineate(object):
    ...
    def getParameterInfo(self):
        ...
        param1 = arcpy.Parameter(
            displayName="If you want to delineate using a LTP derivative, insert the raster here",
            name="inputDer",
            datatype="GPRasterLayer",
    ...
    def execute(self, parameters, messages):
        ...
        cutoffVR = helper.euos_decimalcomma_check(cutoffVR)
        minVR = helper.euos_decimalcomma_check(minVR)
        minWidth = helper.euos_decimalcomma_check(minWidth)
        minRatio = helper.euos_decimalcomma_check(minRatio)
        delBuffer = helper.euos_decimalcomma_check(delBuffer)
        ...
        if parameters[1].value is not None:
            ...
            if fillDirec == "Negative":  
                ...
                saveDer = Negate(inputDer)
                 
            else:
                saveDer = Raster(inputDer)
        ...
        if arcpy.management.GetCount(tempWS + "b3p.shp")[0] == "0":
            ...
            arcpy.SelectLayerByAttribute_management(tempWS + "f7jo_lyr", 
                                                    "NEW_SELECTION", '"Relief" <  %f' %float(minVR))
            ...
            if in_smoosimp == "No simplification or smoothing":
                ...
                # Simplifying
                arcpy.SimplifyPolygon_cartography(tempWS + "g8b_lyr", tempWS + "h9s.shp", "WEIGHTED_AREA", "%f" %float(min_width/10))
                ...
                arcpy.SmoothPolygon_cartography(tempWS + "h9s_lyr", tempWS + "h9z.shp", 
                                                "PAEK", "%f Meters" %float(min_width/5), "NO_FIXED", "NO_CHECK")
                ...
                arcpy.SmoothPolygon_cartography(tempWS + "h9s_lyr", tempWS + "h9z.shp", 
                                                "PAEK", "%f Meters" %float(min_width/2), "NO_FIXED", "NO_CHECK")
...
class Delineate_geomorphons(object):
    ...
    def execute(self, parameters, messages):
        ...
        arcpy.AddMessage("Organising core classes ...")
        ...
        if parameters[3].valueAsText is not None:
            arcpy.AddMessage("Organising subordinate classes ...")
        ...
        if arcpy.management.GetCount(tempWS + "b3p.shp")[0] == "0":
            ...
            if in_smoosimp == "No simplification or smoothing":
                ...
                # Simplifying
                arcpy.SimplifyPolygon_cartography(tempWS + "g8b_lyr", tempWS + "h9s.shp", "WEIGHTED_AREA", "%f" %float(min_width/5))
                arcpy.Delete_management(tempWS + "g8b_lyr")
